				...
				() => {
					...
					if (scavengeTable.length === 0) {
						...
						requests.forEach((request, i) => {
							...
							htmlString += `
								<tr data-row-group-id="${i}">
									<td class="ra-group-text">
											${'Groups: '} ${i+1} ${' -  ( Squads: '} ${squadNum} ${')'}
									</td>
						...
						jQuery('#recalculate').on('click', function (e) {
							e.preventDefault();
							UI.SuccessMessage('Recalculating...');
							if (DEBUG) {
								console.debug(`${scriptInfo()} Recalculating squads`);
							}
							getNewSettings();
							initMassScavenge();
						});
...
function buildUI() {
		if (archerWorld) {
            ...
            <table align="center" class="zscavengeTable" width="100%" style="border: 7px solid rgba(121,0,0,0.71); border-image-slice: 7 7 7 7; border-image-source: url(https://dsen.innogamescdn.com/asset/cf2959e7/graphic/border/frame-gold-red.png);">\
               <tbody>\
                  <tr>\
	...
	htmlString += `
		<table class="ra-table vis" align="center" width="100%">
			<tbody>
	`;
	return htmlString;
}
...
function renderUI(body) {
	...
	const content = `
		...
		<h1 align="center"> <img src="https://dsen.innogamescdn.com/asset/ed7de11c/graphic/unit/unit_militia.png"> ${scriptData.name} <img src="https://dsen.innogamescdn.com/asset/ed7de11c/graphic/unit/unit_militia.png"></h1>
        ...
        <style>
            .ra-mass-scav { position: fixed; z-index: 99999; top: 10vh; right: 10vw; display: block; width: 390px; height: auto; clear: both; margin: 0 auto 15px; padding: 10px; border: 1px solid #603000; box-sizing: border-box; background: #f4e4bc; }
			.ra-mass-scav * { box-sizing: border-box; }
			.custom-close-button { right: 0; top: 0; }
			.ra-table { border-spacing: 2px; border-collapse: separate; border: 2px solid #f0e2be; background-color: #36454f; }
            .ra-table td { center; background-color: #feeeb0;}
            .ra-table td a { word-break: break-all; }
			.ra-text-left { text-align: left !important; }
			.ra-text-center { text-align: center; }
			.ra-group-text { font-size: 1.5em; text-align: left; padding: 5px;}
        </style>
    `;
...
function calculateOptimalSquads(settings, scavengeTable, amountOfPages, durationFactor, durationExponent) {
	...
	for (var page = 0; page <= amountOfPages; page++) {
		...
		for (var i = 0; i < scavengeTables[page].length; i++) {
			...
			var initialized = initInitialDistribution(settings["unit_options"], scavengeTables[page][i].unit_counts);
...
function sendSquadGroups(requests) {
	jQuery('.btn-group-scav').on('click', function (e) {
		e.preventDefault();
		if (DEBUG) {
			console.debug(`${scriptInfo()} Sending squads...`);
		}
